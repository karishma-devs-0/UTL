...
export function PlantsYearlyPVBarChart({ width, height, selectedYear }) {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.post(
          "https://utlsolarrms.com/api/charts/solar_power_per_plant_logs/yearly/",
        ...
        console.log(" Plants Yearly API SUCCESS ✅", response.data);
  ...
  const yTicks = useMemo(() => {
    ...
    return Array.from(
      { length: TICK_COUNT + 1 },
      (_, i) => Number((step * i).toFixed(2)) // 👈 KEEP DECIMALS
    );
  }, [Y_MAX]);
  const formatY = (v) => {
  ...
  return (
    <View style={{ width, height }}>
      <Canvas style={{ width, height }}>
        ...
        {/* Bars */}
        {data.map((d) => {
          const x = getXCenter(d.month) - BAR_WIDTH / 2;
