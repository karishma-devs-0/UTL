...
const deviceService = {
  ...
  getMainDevices: async (plantId = '1') => {
    ...
    try {
      ...
      console.error('Error fetching main devices (inverters):', error.response?.data || error.message);
  ...
  getLoggerData: async (plantId) => {
    try {
      ...
      console.error(`Error fetching logger data for plant ${plantId}:`, error.response?.data || error.message);
      return { success: false, error: error.response?.data?.message || error.message };
    }
  },
  ...
  getPlantDashboardDetails: async (plantId) => {
    if (!plantId) {
      return { success: false, error: 'Plant ID is required for dashboard details.' };
    }
    try {
      const response = await apiClient.get('/plantdashboard/', { params: { id: plantId } });
      ...
      if (response.data && Array.isArray(response.data) && response.data.length > 0) {
        ...
        return { success: false, error: 'Plant not found for dashboard details.' };
      } else {
        return { success: false, error: 'Unexpected response structure.' };
      }
    } catch (error) {
      if (!error.response) {
        console.error(`Error fetching plant dashboard details for plantId ${plantId} (network):`, error.message);
        return { success: false, error: error.message || 'Network error fetching plant dashboard details.' };
      }
      console.error(`Error fetching plant dashboard details for plantId ${plantId}:`, error.response?.data || error.message);
      return { success: false, error: error.response?.data?.message || error.message || 'Failed to fetch plant dashboard details.' };
    }
  },
  // --- END NEW FUNCTION ---
  ...
  getDashboardSummary: async () => {
    try {
      ...
      if (response.data && response.data.success) { // Adjust if success flag is different
        return { success: true, data: response.data.summary || response.data.data || response.data }; 
      } else {
        return { success: false, error: response.data?.message || 'Failed to retrieve dashboard summary.' };
      }
    } catch (error) {
      console.error(`Error fetching dashboard summary:`, error.response?.data || error.message);
      return { success: false, error: error.response?.data?.message || error.message || 'Failed to fetch dashboard summary.' };
    }
  },
  // --- END NEW FUNCTION ---
  ...
  getInverterData: async (plantId) => {
    try {
      ...
      console.log(`[deviceService.getInverterData] Fetching using devices API for plantId: ${plantId}`);
      
      // First try using the devices API which returns data for all inverters
      const response = await apiClient.get('/devices/');
      
      console.log(`[deviceService.getInverterData] Devices API response:`, 
        JSON.stringify(response.data, null, 2).substring(0, 500) + '...');
      
      let mappedData = [];
      
      if (response.data && response.data.success && Array.isArray(response.data.inverter)) {
        // Filter by plantId if provided
        const filteredInverters = plantId ? 
          response.data.inverter.filter(item => item.plantId == plantId || item.plant_id == plantId) : 
          response.data.inverter;
          
        console.log(`[deviceService.getInverterData] Found ${filteredInverters.length} inverters for plant ${plantId}`);
        ...
        if (mappedData.length > 0) {
          console.log('[deviceService.getInverterData] First mapped inverter from devices API:',
      ...
      console.log('[deviceService.getInverterData] Devices API returned no data, falling back to log_invert');
      ...
      if (fallbackResponse.data && Array.isArray(fallbackResponse.data.data)) {
        ...
        console.log(`[deviceService.getInverterData] Fallback API returned ${mappedData.length} inverters`);
        
        if (mappedData.length > 0) {
          console.log('[deviceService.getInverterData] First mapped inverter from fallback:',
            JSON.stringify(mappedData[0], null, 2));
        }
      ...
      return {
        success: false,
        error: error.response?.data?.message || error.message || 'Unknown error fetching inverter data'
      };
    }
  },
  ...
  updatePlant: async (plantData) => {
    ...
    try {
        ...
        console.error(`Error updating plant ${plantData.id}:`, error.response?.data || error.message, error);
        const errorMessage = error.response?.data?.message || error.message || 'Failed to update plant.';
  ...
  deletePlant: async (plantId) => {
    ...
    try {
        ...
        console.error('[deviceService] Error deleting plant:', error.response ? error.response.data : error.message);
        return { success: false, error: error.response?.data?.message || error.message || 'Failed to delete plant' };
    }
  },
  ...
  createPlant: async (plantPayload) => {
    ...
    console.log('[deviceService.createPlant] Payload just before sending:', JSON.stringify(payloadToSend, null, 2));
    ...
    try {
        ...
        console.error('Error creating plant:', error.response?.data || error.message, error);
        const errorMessage = error.response?.data?.message || error.message || 'Failed to create plant.';
  ...
  deleteLogger: async (loggerSno) => {
    if (!loggerSno) {
        return { success: false, error: 'Logger Serial Number (sno) is required for deletion.' };
    }
    try {
        ...
        console.error(`Error deleting logger ${loggerSno}:`, error.response?.data || error.message, error);
        // Provide a user-friendly error message
        const errorMessage = error.response?.data?.message || error.message || 'Failed to delete logger.';
        return { success: false, error: errorMessage };
    }
  },
  ...
  getLoggerDetailsBySno: async (sno, plantId) => {
    ...
    if (typeof plantId === 'undefined' || plantId === null) { // Check for null explicitly
    ...
    try {
      ...
      else if (response.data && (response.data.success === true || response.data.success === 'true') && (response.data.data || response.data.results)) {
        const loggerData = response.data.data || (Array.isArray(response.data.results) ? response.data.results[0] : response.data.results);
        if (loggerData) {
            return { success: true, data: loggerData };
        } else {
            return { success: false, error: 'No logger data found for this S/N and Plant ID.' };
        }
      } 
      // Handle other unexpected successful responses or empty arrays from the new check
      else if (response.status === 200) {
        return { success: false, error: 'Logger details not found or in unexpected format.' };
      }
      // This will now mostly catch non-200 responses if not caught by axios default error handling
      else {
        return { success: false, error: response.data?.message || 'Failed to retrieve logger details by S/N for given Plant ID.' };
      }
    } catch (error) {
      const errorMessage = error.response?.data?.message || error.message || 'An unexpected error occurred while fetching logger details.';
      return { success: false, error: errorMessage };
    }
  },
  // --- END FUNCTION ---
  ...
  getLoggerDetailsByMacAddress: async (macAddress) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`HTTP error ${response.status}: ${errorBody || 'Failed to fetch logger details'}`);
      }
      const data = await response.json();
      if (data.success && data.data && data.data.length > 0) {
        return { success: true, data: data.data[0] };
      } else if (data.success && (!data.data || data.data.length === 0)) {
        return { success: false, error: 'Logger not found or no data available.' };
      } else {
        return { success: false, error: data.message || 'Failed to retrieve logger details' };
      }
    } catch (error) {
  ...
  getDailyPowerForPlant: async (plantId, date) => { // date format: YYYY-MM-DD
    if (!plantId || !date) {
        return { success: false, error: 'Plant ID and Date (YYYY-MM-DD) are required.' };
    }
    try {
        ...
        const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch daily chart data.';
        return { success: false, error: errorMessage, results: [] }; 
    }
  },
  // --- End Function for Daily Chart Data ---

  getDailyPowerForPlantLogs: async (plantId, date, loggerSno) => { // date format: YYYY-MM-DD
    if (!plantId || !date || !loggerSno) {
      return { success: false, error: 'Plant ID, Date (YYYY-MM-DD), and loggerSno are required.' };
    }
    try {
      const endpoint = `/charts/solar_power_per_plant_logs/daily/`;
      const payload = {
        plant_id: parseInt(plantId, 10),
        date_parameter: date,
        loggerSno: String(loggerSno)
      };
      const response = await apiClient.post(endpoint, payload);
      return { success: true, results: response.data?.results || [] };
    } catch (error) {
      const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch daily chart data (logs).';
      return { success: false, error: errorMessage, results: [] };
    }
  },
  ...
  getMonthlyProductionForPlant: async (plantId, monthYear) => { // monthYear format: "YYYY-MM"
    if (!plantId || !monthYear) {
        return { success: false, error: 'Plant ID and Month (YYYY-MM) are required.' };
    }
    ...
    try {
        ...
        const response = await apiClient.post(endpoint, payload, { timeout: 60000 }); // Timeout set to 60 seconds
        // -------------------------------------------------------------------------- -----------
        ...
        if (error.response) {
            ...
            console.error(`[iOS DEBUG] Server Error Status: ${error.response.status}`);
            console.error(`[iOS DEBUG] Server Error Data: ${JSON.stringify(error.response.data)}`);
        } else if (error.request) {
            // Request was made but no response received (e.g., network error, timeout)
            console.error('[iOS DEBUG] Network error or no response received:', error.message);
        } else {
            // Other errors (e.g., setup issue)
            console.error('[iOS DEBUG] Other error:', error.message);
        }
        const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch monthly chart data.';
        return { success: false, error: errorMessage, results: [] };
    }
  },
  ...
  getYearlyProductionForPlant: async (plantId, year) => {
    if (!plantId || !year) {
      return { success: false, error: 'Plant ID and Year (YYYY) are required.' };
    }
    try {
      ...
      const results = response.data?.results?.map(item => ({
        ...item,
        solarPower: item.solarPower ? (parseFloat(item.solarPower) / 1000).toFixed(3) : '0.000' // Convert to MWh
      })) || [];
      
      return { success: true, results };
    } catch (error) {
      const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch yearly chart data.';
      return { success: false, error: errorMessage, results: [] }; 
    }
  },
  ...
  getTotalProductionForPlant: async (plantId) => { 
    ...
    try {
        ...
        const results = response.data?.results?.map(item => ({
          ...
          solarPower: item.solarPower ? Number((parseFloat(item.solarPower) / 1000).toFixed(3)) : 0 // Convert to MWh as number
        })) || [];
        
        return { success: true, results };
    } catch (error) {
        const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch total chart data.';
        return { success: false, error: errorMessage, results: [] }; 
    }
  },
  ...
  getAllDevicesSummary: async () => {
    try {
      console.log('[deviceService] Fetching all devices summary from /api/devices/');
      const response = await apiClient.get('/devices/');
      ...
      if (response.data && response.data.success) {
        ...
        console.error('[deviceService] API returned unsuccessful response:', response.data);
        return {
            success: false,
      ...
      return {
        success: false,
        error: error.message || 'An error occurred while fetching devices summary'
      };
    }
  },
  // --- END ADDED function ---
  ...
  getDailyPowerForDevice: async (deviceSn, date) => {
    if (!deviceSn || !date) {
      return { success: false, error: 'Device S/N and Date (YYYY-MM-DD) are required.' };
    }
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch daily chart data for device.';
      return { success: false, error: errorMessage, results: [] };
    }
  },
  ...
  getMonthlyProductionForDevice: async (deviceSn, monthYear) => {
    if (!deviceSn || !monthYear) {
      return { success: false, error: 'Device S/N and Month (YYYY-MM) are required.' };
    }
    try {
      const endpoint = '/charts/devices/monthly/device_month_chart';
      const payload = { device_sn: String(deviceSn), date_parameter: monthYear };
      const response = await apiClient.post(endpoint, payload);
      return { success: true, results: response.data?.results || [] };
    } catch (error) {
      const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch monthly chart data for device.';
      return { success: false, error: errorMessage, results: [] };
    }
  },
  ...
  getYearlyProductionForDevice: async (deviceSn, year) => {
    if (!deviceSn || !year) {
      return { success: false, error: 'Device S/N and Year (YYYY) are required.' };
    }
    try {
      const endpoint = '/charts/devices/yearly/device_year_chart';
      const payload = { device_sn: String(deviceSn), date_parameter: String(year) };
      const response = await apiClient.post(endpoint, payload);
      return { success: true, results: response.data?.results || [] };
    } catch (error) {
      const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch yearly chart data for device.';
      return { success: false, error: errorMessage, results: [] };
    }
  },
  ...
  getTotalProductionForDevice: async (deviceSn) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch total chart data for device.';
      return { success: false, error: errorMessage, results: [] };
    }
  },
  ...
  getInverterParameters: async (loggerSno, plantId) => { // Renamed inverterSno to loggerSno for clarity
    if (!loggerSno) { // Check loggerSno
      const errorMessage = 'Logger S/N is required to fetch inverter parameters.'; // Updated error message
      return { success: false, error: errorMessage };
    }
    try {
      ...
      if (data.success && data.data) { // Check for data.data as per your successful response structure
        return { success: true, data: data.data }; 
      } else {
        return { success: false, error: data.message || 'API returned success=false or missing data object' };
      }
    } catch (error) {
      return { 
        success: false, 
        error: error.response?.data?.message || error.message || 'An unexpected error occurred while fetching inverter parameters.' 
      };
    }
  },
  ...
  getInvertersForPlant: async (plantId) => {
    ...
    console.log(`[deviceService.getInvertersForPlant] Fetching inverters for plant ${plantId}`);
    ...
    try {
      ...
      if (response.data && response.data.success && Array.isArray(response.data.inverter)) {
        ...
        if (plantInverters.length > 0) {
          console.log(`[deviceService.getInvertersForPlant] Found ${plantInverters.length} inverters from devices API`);
      ...
      console.log(`[deviceService.getInvertersForPlant] Trying plantdashboard API for inverter data`);
      const dashboardResponse = await apiClient.get('/plantdashboard', { params: { id: plantId } });
      
      if (dashboardResponse.data && Array.isArray(dashboardResponse.data) && dashboardResponse.data.length > 0) {
        ...
        if (plant.inverters && Array.isArray(plant.inverters)) {
          console.log(`[deviceService.getInvertersForPlant] Found ${plant.inverters.length} inverters in dashboard API`);
          return { success: true, data: plant.inverters };
        }
      }
      
      // Last resort - try log_invert endpoint directly
      console.log(`[deviceService.getInvertersForPlant] Trying log_invert API as last resort`);
      const logInvertResponse = await apiClient.post('/log_invert', { plantId });
      
      if (logInvertResponse.data && Array.isArray(logInvertResponse.data.data)) {
        console.log(`[deviceService.getInvertersForPlant] Found ${logInvertResponse.data.data.length} inverters from log_invert`);
        return { success: true, data: logInvertResponse.data.data };
      }
      
      // If we get here, no data was found in any endpoint
      return { success: false, error: 'No inverter data found for this plant.' };
    } catch (error) {
      console.error('[deviceService.getInvertersForPlant] Error:', error);
      return {
        success: false,
        error: error.response?.data?.message || error.message || 'Failed to fetch inverter data for plant'
      };
    }
  },
  // --- END NEW FUNCTION ---
