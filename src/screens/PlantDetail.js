...
import { apiGetplantdashboard, getPlantStatus } from "@/utils/services/plantDetailService";
...
const validateChartData = (chartData) => {
  ...
  if (!Array.isArray(chartData.labels)) {
    console.warn('[Chart Validation] Invalid chart data: labels is not an array');
    return false;
  }
  
  if (!Array.isArray(chartData.datasets)) {
    console.warn('[Chart Validation] Invalid chart data: datasets is not an array');
    return false;
  }
  ...
  if (chartData.labels.length < 2) {
    console.warn('[Chart Validation] Invalid chart data: need at least 2 data points for SVG path');
    return false;
  }
  
  for (const dataset of chartData.datasets) {
    if (!Array.isArray(dataset.data)) {
      console.warn('[Chart Validation] Invalid chart data: dataset.data is not an array');
      return false;
    }
    
    if (dataset.data.length !== chartData.labels.length) {
      console.warn('[Chart Validation] Invalid chart data: data length does not match labels length');
      return false;
    }
    ...
    for (let i = 0; i < dataset.data.length; i++) {
      ...
      if (val === null || val === undefined) {
        ...
        if (consecutiveNulls > Math.floor(dataset.data.length / 2)) {
          console.warn(`[Chart Validation] Too many consecutive null values at index ${i}`);
          return false;
        }
      } else {
        consecutiveNulls = 0;
        ...
        if (isNaN(val) || typeof val !== 'number') {
          console.warn(`[Chart Validation] Invalid chart data: data point at index ${i} is not a valid number: ${val}`);
          return false;
        }
        
        // Check for extreme values that could cause SVG issues
        if (!isFinite(val) || Math.abs(val) > 1e15) {
          console.warn(`[Chart Validation] Invalid chart data: extreme value at index ${i}: ${val}`);
          return false;
        }
    ...
    if (validDataCount < 1) {
      console.warn('[Chart Validation] Invalid chart data: no valid data points found');
      return false;
    }
  }
...
const sanitizeLineChartData = (chartData) => {
  ...
  const sanitizedData = originalData.map((value, index) => {
    if (value === null || value === undefined || isNaN(value) || !isFinite(value)) {
      console.warn(`[LineChart Sanitization] Replacing invalid value at index ${index}: ${value} with 0`);
      return 0;
    }
    
    // Clamp extremely large values that could cause SVG issues
    if (Math.abs(value) > 1e12) {
      console.warn(`[LineChart Sanitization] Clamping extreme value at index ${index}: ${value}`);
    ...
    datasets: [{
      ...
      ...(chartData.visualData && {
        visualData: chartData.visualData.map(value => {
          ...
          return Math.abs(value) > 1e12 ? (value > 0 ? 1e12 : -1e12) : Number(value);
        })
      })
    }]
  };
...
const sanitizeBarChartData = (chartData) => {
  ...
  const sanitizedData = originalData.map((value, index) => {
    if (value === null || value === undefined || isNaN(value) || !isFinite(value)) {
      console.warn(`[BarChart Sanitization] Replacing invalid value at index ${index}: ${value} with 0`);
      return 0;
    }
    
    // Clamp extremely large values that could cause SVG issues
    if (Math.abs(value) > 1e12) {
      console.warn(`[BarChart Sanitization] Clamping extreme value at index ${index}: ${value}`);
      return value > 0 ? 1e12 : -1e12;
    }
    
    // Ensure non-negative values for bar charts (negative bars can cause SVG issues)
    if (value < 0) {
      console.warn(`[BarChart Sanitization] Converting negative value to 0 at index ${index}: ${value}`);
      return 0;
    }
              ...
              <View style={styles.statItem}>
                ...
                <Text style={styles.statValue}>
                  {(() => {
                    ...
                    console.log('Yearly Production raw:', plant.yearlyProduction, 'formatted:', formatted);
                    return formatted.value;
                  })()}
                  <Text style={styles.statUnit}> {(() => {
                    if (plant?.yearlyProduction === undefined || plant?.yearlyProduction === null || plant?.yearlyProduction === '--') {
...
const PlantDetailScreen = ({ route, navigation }) => {
  ...
  const [monthLatestProduction, setMonthLatestProduction] = useState(null);
  const [yearLatestProduction, setYearLatestProduction] = useState(null);
  ...
  const [dayLatestNonZeroPowerWatts, setDayLatestNonZeroPowerWatts] = useState(null);
  ...
  const [plantLoggerSno, setPlantLoggerSno] = useState(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const loggersArray = Array.isArray(result?.data)
          ? result.data
        ...
        const firstLogger = loggersArray[0] || null;
        const sno = firstLogger?.sno ?? firstLogger?.loggerSno ?? firstLogger?.logger_sno ?? firstLogger?.serialNumber ?? firstLogger?.serial_number;
        setPlantLoggerSno(sno ? String(sno) : null);
      } catch (e) {
        if (!isMounted) return;
        setPlantLoggerSno(null);
      }
  ...
  const fetchDayChartData = async (dateToFetch) => {
    ...
    try {
      ...
      let result;
      if (plantLoggerSno) {
        result = await deviceService.getDailyPowerForPlantLogs(plantId, dateParam, plantLoggerSno);
        if (!result?.success) {
          result = await deviceService.getDailyPowerForPlant(plantId, dateParam);
        }
      } else {
        result = await deviceService.getDailyPowerForPlant(plantId, dateParam);
      }
    ...
    try {
      ...
      if (result && result.results && Array.isArray(result.results)) {
        ...
        apiData.forEach(item => {
          if (!item || item.PvProduction === null || item.PvProduction === undefined || item.PvProduction === '--') {
            return;
          }
        ...
        let latestProductionValue = null;
        for (let i = dailyCounts.length - 1; i >= 0; i--) {
          if (dailyCounts[i] > 0) {
            latestProductionValue = dailyTotals[i];
            break;
          }
        }
        setMonthLatestProduction(latestProductionValue);
    ...
    try {
      ...
      if (response.data?.results && Array.isArray(response.data.results)) {
        ...
        response.data.results.forEach(item => {
          if (!item || item.month === null || item.month === undefined) return;
          const monthIndex = parseInt(item.month, 10) - 1;
          if (isNaN(monthIndex) || monthIndex < 0 || monthIndex > 11) return;
          if (item.PvProduction === null || item.PvProduction === undefined || item.PvProduction === '--') return;
          const production = parseFloat(item.PvProduction);
          if (isNaN(production)) return;
          monthTotals[monthIndex] += production;
          monthCounts[monthIndex] += 1;
        });

        let latestProductionValue = null;
        for (let i = monthCounts.length - 1; i >= 0; i--) {
          if (monthCounts[i] > 0) {
            latestProductionValue = monthTotals[i];
            break;
          }
        }
        setYearLatestProduction(latestProductionValue);

        // Find the maximum value to scale the y-axis properly
        const maxValue = Math.max(...monthTotals, 0.1);
        ...
        const yearData = {
          ...
          datasets: [{
            data: monthLabels.map((_, index) => {
              ...
              const value = monthTotals[index] || 0;
    ...
    try {
      ...
      if (response.data?.results && Array.isArray(response.data.results) && response.data.results.length > 0) {
        ...
        if (filteredApiData.length > 0) {
          ...
          const colors = data.map(() => (opacity = 1) => `rgba(220, 53, 69, ${opacity})`); // Standardized red
          ...
          setTotalChartError('No data available');
        }
      } else {
        setTotalChartData(null);
        setTotalChartError('No data available');
  ...
  }, [selectedProductionTab, selectedDate, plantId, plantLoggerSno]); // Add selectedDate as a dependency
  ...
  useEffect(() => {
    if (selectedProductionTab === 'Day' && plantDayRawData && !isDayChartLoading) {
      ...
      if (selectedPlantDayParameterId === 'productionPower') {
        ...
        plantDayRawData.forEach(item => {
          const currentPowerW = parseFloat(item?.PvProduction ?? item?.solarPower ?? item?.pvProduction);
        ...
        plantDayRawData.forEach(item => {
          const powerW = parseFloat(item?.PvProduction ?? item?.solarPower ?? item?.pvProduction); // Watts
      ...
      for (const item of plantDayRawData) {
        const v = paramConfig?.dataExtractor ? paramConfig.dataExtractor(item) : parseFloat(item?.PvProduction ?? item?.solarPower ?? item?.pvProduction);
  ...
  switch (selectedProductionTab) {
    case 'Day':
            ...
            <View style={styles.miniCardsContainer}>
              <View style={styles.miniCard}>
                ...
                <Text style={styles.miniCardValue}>
                  {(() => {
                    if (dayLatestNonZeroPowerWatts === null || dayLatestNonZeroPowerWatts === undefined) return '--';
              ...
              <View style={styles.miniCard}>
                ...
                <Text style={styles.miniCardValue}>
                  {plantDashboardData?.peak_hours_today ? `${plantDashboardData.peak_hours_today} h` : '--'}
                </Text>
              </View>
            </View>
            ...
            <>
              <View style={styles.miniCardsContainer}>
                <View style={styles.miniCard}>
                  ...
                  <Text style={styles.miniCardValue}>
                    {(() => {
                      if (monthLatestProduction === null || monthLatestProduction === undefined) return '--';
                      if (Number(monthLatestProduction) === 0) return '0.00 kWh';
                      const formatted = formatEnergyWithUnit(monthLatestProduction);
              ...
              <View style={styles.graphLegendContainer}>
                <Text style={styles.graphLegendText}>Production (kWh)</Text>
              </View>
              ...
              <View style={{ position: 'relative' }}>
                ...
                >
                  ...
                  >
                    <BarChart
                      data={sanitizeBarChartData(getHighlightedChartData(monthChartData, 'Month'))}
            ...
            <>
              <View style={styles.dailySummaryContainer}>
                <View style={styles.summaryCard}>
                  ...
                  <Text style={styles.summaryValue}>
                    {(() => {
                      if (yearLatestProduction === null || yearLatestProduction === undefined) return '--';
                      const numericValue = Number(yearLatestProduction);
                      if (Number.isNaN(numericValue)) return '--';
                      if (numericValue === 0) return '0.00 kWh';
                      const formatted = formatEnergyWithUnit(numericValue);
                      return `${formatted.value} ${formatted.unit}`;
                    })()}
                  </Text>
                </View>
              ...
              <View style={styles.graphLegendContainer}>
                <Text style={styles.graphLegendText}>
                  {(() => {
                    const legendText = Array.isArray(yearChartData?.legend) ? yearChartData.legend[0] : '';
                    const match = typeof legendText === 'string' ? legendText.match(/\(([^)]+)\)/) : null;
                    const unit = match?.[1] || 'kWh';
              ...
              <View style={{ position: 'relative' }}>
                ...
                >
                  ...
                  >
                    <BarChart
                      data={sanitizeBarChartData(getHighlightedChartData(yearChartData, 'Year'))}
      ...
      case 'Total':
        ...
        if (totalChartData && (totalChartData?.labels?.length === 1 || validateChartData(totalChartData))) {
          chartToRender = (
            <>
              <View style={styles.dailySummaryContainer}>
                <View style={styles.summaryCard}>
              ...
              <View style={styles.graphLegendContainer}>
                <Text style={styles.graphLegendText}>
                  {(() => {
                    const legendText = Array.isArray(totalChartData?.legend) ? totalChartData.legend[0] : '';
                    const match = typeof legendText === 'string' ? legendText.match(/\(([^)]+)\)/) : null;
                    const unit = match?.[1] || 'kWh';
                    return `Production (${unit})`;
                  })()}
              ...
              <View style={{ position: 'relative' }}>
                ...
                >
                  ...
                  >
                    <BarChart
                      data={sanitizeBarChartData(getHighlightedChartData({
                        ...totalChartData,
        ...
        } else if (totalChartError) {
          chartToRender = (
            <View style={styles.chartLoadingContainer}>
              <Text style={styles.chartErrorText}>{totalChartError}</Text>
            </View>
          );
        }
        break;
          ...
          <View style={styles.detailContainer}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>
                {plant?.capacity ? (
                  <>
                    {plant.capacity}<Text style={styles.unitText}> kWp</Text>
                  </>
                ) : (
