...
const PlantDetailScreen = ({ route, navigation }) => {
  ...
  const fetchPlantDashboardData = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `https://utlsolarrms.com/api/plantdashboard?id=${plantId}`
      );
    ...
    try {
      ...
      if (result.results && Array.isArray(result.results)) {
        ...
        apiData.forEach((item) => {
          if (
            !item ||
            item.solarPower === null ||
            item.solarPower === undefined ||
            item.solarPower === "--"
          ) {
            return;
          }
  ...
  const handleBarChartClick = (event, chartType) => {
    ...
    if (chartType === "Month") {
      chartWidth = Math.max(
        cardWidth - 32,
        (monthChartData?.labels?.length || 31) * 20
      );
      ...
      chartWidth = Math.max(
        cardWidth - 32,
        (yearChartData?.labels?.length || 12) * 40
      );
    ...
    if (
      ...
      monthChartData.datasets[0] &&
      monthChartData.datasets[0].data
    ) {
      ...
      chartType === "Year" &&
      yearChartData &&
      yearChartData.datasets &&
      yearChartData.datasets[0] &&
      yearChartData.datasets[0].data
    ) {
      ...
      chartType === "Total" &&
      totalChartData &&
      totalChartData.datasets &&
      totalChartData.datasets[0] &&
    ...
    if (data !== undefined) {
      ...
      const tooltipX = Math.max(
        TOOLTIP_WIDTH / 2,
        Math.min(pageX, screenWidth - TOOLTIP_WIDTH / 2)
      );
      ...
      console.log("Manual tooltip set:", {
        label,
        displayValue,
        x: tooltipX,
        y: tooltipY,
                ...
                <View style={styles.miniCard}>
                  ...
                  <Text style={styles.miniCardValue}>
                    {(() => {
                      ...
                      const formatted = formatEnergyWithUnit(
                        plantDashboardData.daily_production
                      );
                ...
                <View style={styles.miniCard}>
                  ...
                  <Text style={styles.miniCardValue}>
                    {plantDashboardData?.peak_hours_today
                      ? `${plantDashboardData.peak_hours_today} kW`
                      : "--"}
                  </Text>
                </View>
              </View>
                ...
                <LineChart
                  ...
                  chartConfig={{
                    ...
                    propsForBackgroundLines: {
                      ...
                      strokeWidth:
                        dayChartData &&
                        dayChartData.datasets &&
                        dayChartData.datasets[0].data.some(
                          (val) => val !== null
                        )
                          ? 0.8
                          : 0, // Hide grid if no data
                    },
                          ...
                          datasets:
                            totalChartData?.datasets?.map((dataset) => ({
                              ...
                              data:
                                ...
                                [],
                              color: (opacity = 1) =>
                                `rgba(0, 150, 50, ${opacity})`, // Ensure color is a function
                            })) || [],
                        },
                        "Total"
                      )}
