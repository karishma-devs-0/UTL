...
const PLANT_DAY_PARAMETERS = {
  productionPower: {
    ...
    dataExtractor: (item) => parseFloat(item.solarPower), // Use raw Watts for chart
    summaryExtractor: (item) => parseFloat(item.solarPower), // Keep in W for summary
    summaryUnit: 'W',
        ...
        <View style={styles.modalContent}>
          <View style={styles.modalHeader}>
            <TouchableOpacity onPress={onClose}>
              <Text style={[styles.modalCloseText, { color: '#00BCD4' }]}>Close</Text>
            </TouchableOpacity>
          ...
          <ScrollView style={styles.modalScrollView}>
            ...
            <View style={styles.chartDetailContainer}>
              ...
              <View style={styles.legendContainer}>
                <View style={styles.legendItem}>
                  <View style={[styles.legendDot, { backgroundColor: '#e0f2f1' }]} />
            ...
            <View style={styles.chartInfoContainer}>
              <Text style={styles.chartInfoText}>
                {(() => {
                  const formatted = formatEnergyWithUnit(plant?.monthlyProduction || '0.00');
            ...
            <View style={styles.statsContainer}>
              <View style={styles.statItem}>
                ...
                <Text style={styles.statValue}> 
                  ...
                  {(plant?.solar_power !== undefined && plant?.solar_power !== null && plant?.solar_power !== '--') ? 
              ...
              <View style={styles.statItem}>
                ...
                <Text style={styles.statValue}>
                  {(() => {
                    if (plant?.daily_production === undefined || plant?.daily_production === null || plant?.daily_production === '--') {
                      return '--';
                    }
                    const formatted = formatEnergyWithUnit(plant.daily_production);
              ...
              <View style={styles.statItem}>
                ...
                <Text style={styles.statValue}>
                  {(() => {
                    if (plant?.monthlyProduction === undefined || plant?.monthlyProduction === null || plant?.monthlyProduction === '--') {
                      return '--';
                    }
                    const formatted = formatEnergyWithUnit(plant.monthlyProduction);
                    return formatted.value;
                  })()}
                  <Text style={styles.statUnit}> {(() => {
                    if (plant?.monthlyProduction === undefined || plant?.monthlyProduction === null || plant?.monthlyProduction === '--') {
              ...
              <View style={styles.statItem}>
                ...
                <Text style={styles.statValue}>
                  {(() => {
                    if (plant?.yearlyProduction === undefined || plant?.yearlyProduction === null || plant?.yearlyProduction === '--') {
                      return '--';
                    }
                    const formatted = formatEnergyWithUnit(plant.yearlyProduction);
                    return formatted.value;
                  })()}
                  <Text style={styles.statUnit}> {(() => {
                    if (plant?.yearlyProduction === undefined || plant?.yearlyProduction === null || plant?.yearlyProduction === '--') {
              ...
              <View style={[styles.statItem, { borderBottomWidth: 0 }]}>
                ...
                <Text style={styles.statValue}>
                  {(() => {
                    if (plant?.totalProduction === undefined || plant?.totalProduction === null || plant?.totalProduction === '--') {
                      return '--';
                    }
                    const formatted = formatEnergyWithUnit(plant.totalProduction);
                    return formatted.value;
                  })()}
                  <Text style={styles.statUnit}> {(() => {
                    if (plant?.totalProduction === undefined || plant?.totalProduction === null || plant?.totalProduction === '--') {
        ...
        { 
          ...
          onPress: async () => {
            ...
            if (result.success) {
              ...
              console.error(`Failed to delete plant ${plantIdSafe}:`, result.error);
...
const PlantDetailScreen = ({ route, navigation }) => {
  ...
  const [plantStatus, setPlantStatus] = useState({ color: '#9E9E9E', icon: 'help', text: 'Loading...' });
  ...
  const [selectedPlantDayParameterId, setSelectedPlantDayParameterId] = useState('productionPower');
  const [isPlantDayParameterModalVisible, setPlantDayParameterModalVisible] = useState(false);
  const [tempSelectedPlantDayParameterId, setTempSelectedPlantDayParameterId] = useState('productionPower');
  const [plantDaySummaryValue, setPlantDaySummaryValue] = useState('--');
  const [plantDaySummaryUnit, setPlantDaySummaryUnit] = useState(PLANT_DAY_PARAMETERS.productionPower.summaryUnit);
  
  // Add missing state variables for other tabs
  const [selectedPlantMonthParameterId, setSelectedPlantMonthParameterId] = useState('productionPower');
  const [selectedPlantYearParameterId, setSelectedPlantYearParameterId] = useState('productionPower');
  const [selectedPlantTotalParameterId, setSelectedPlantTotalParameterId] = useState('productionPower');
  ...
  // Tooltip State
  const [tooltipPos, setTooltipPos] = useState({ x: 0, y: 0, visible: false, value: null, label: '' });
  ...
  // NEW STATE VARIABLES
  const [calculatedDailyProductionKWh, setCalculatedDailyProductionKWh] = useState(null);
  ...
  const fetchPlantStatus = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        } else if (data.data.partiallyOffline.plantIds.includes(currentPlantId)) {
          setPlantStatus({ 
  ...
  const toggleFavorite = useCallback(async () => {
    if (!plant || (!plant._id && !plant.id)) {
      Alert.alert('Error', 'Plant data not available to update favorite status.');
      return;
    }
    const currentPlantId = plant._id || plant.id;
    ...
    const result = await dataManager.setPlantFavoriteStatus(currentPlantId, newFavoriteStatus);

    if (!result.success) {
      setIsFavorite(!newFavoriteStatus);
      Alert.alert('Error', result.error || 'Failed to update favorite status.');
    } else {
      setPlant(prevPlant => prevPlant ? { ...prevPlant, isFavorite: newFavoriteStatus } : null);
    }
  }, [plant, isFavorite]);
  ...
  const getSelectedMonthYearString = () => selectedDate.toISOString().slice(0, 7);
  ...
  const fetchDayChartData = async (dateToFetch) => {
    ...
    const currentParamConfig = PLANT_DAY_PARAMETERS[selectedPlantDayParameterId];
    ...
    try {
      const result = await deviceService.getDailyPowerForPlant(plantId, dateParam);
      
      if (result.success && result.results) { // No need to check length here, useEffect will handle empty
        setPlantDayRawData(result.results || []);
        setDayChartError(null);
      } else {
        console.warn(`[PlantDetail] No success or no results for DAY data on ${dateParam}. Error: ${result.error}`);
        setPlantDayRawData([]);
      ...
      console.error(`[PlantDetail] Error fetching DAY data for ${dateParam}:`, err);
      setPlantDayRawData([]);
  ...
  const fetchMonthChartData = async () => {
    ...
    console.log(`[Month Chart] Fetching data for ${currentYear}-${currentMonth}`);
    
    const daysInSelectedMonth = new Date(currentYear, currentMonth, 0).getDate();
    const monthPlaceholderLabels = Array.from({ length: daysInSelectedMonth }, (_, i) => {
        const day = i + 1;
        return (day === 1 || day === daysInSelectedMonth || day % 5 === 0) ? String(day) : '';
    });
    ...
    try {
      ...
      const response = await apiClient.post('/charts/solar_power_per_plant/monthly/', {
        plant_id: plantId, 
      ...
      if (result.results && Array.isArray(result.results)) {
        ...
        apiData.forEach(item => {
          if (!item || item.solarPower === null || item.solarPower === undefined || item.solarPower === '--') {
            return;
          }
          const dayIndex = parseInt(item.date, 10) - 1;
          if (isNaN(dayIndex) || dayIndex < 0 || dayIndex >= daysInSelectedMonth) {
            return;
          }
          const productionWh = parseFloat(item.solarPower);
        ...
        const legendUnit = maxValue > 1000 ? "MWh" : "kWh";
      ...
      setMonthChartError("An unexpected error occurred while fetching monthly data.");
      setMonthChartData(defaultEmptyData);
    } finally {
      setIsMonthChartLoading(false);
  ...
  const fetchYearChartData = async () => {
    ...
    try {
      ...
      const response = await apiClient.post('/charts/solar_power_per_plant/yearly/', {
        plant_id: plantId,
        date_parameter: dateParameter
      });

      console.log('Year Chart API Response:', JSON.stringify(response.data, null, 2));
      ...
      if (response.data?.results && Array.isArray(response.data.results)) {
        ...
        response.data.results.forEach(item => {
          ...
          monthDataMap[item.month - 1] = parseFloat(item.solarPower || 0);
        });
        ...
        const yearData = {
          ...
          datasets: [{
            ...
            color: (opacity = 1) => `rgba(0, 150, 50, ${opacity})` // Add color function to dataset
          }],
      ...
      setYearChartError(err.response?.data?.message || 'Failed to fetch yearly data');
      setYearChartData(null);
    } finally {
      setIsYearChartLoading(false);
    }
 ...
 const fetchTotalChartData = async () => {
    ...
    try {
      const response = await apiClient.post('/charts/solar_power_per_plant/total/', {
        plant_id: plantId
      });

      console.log('Total Chart API Response:', JSON.stringify(response.data, null, 2));

      if (response.data?.results && Array.isArray(response.data.results) && response.data.results.length > 0) {
        let apiData = response.data.results;
              
        // Filter out invalid years
        const filteredApiData = apiData.filter(item => parseInt(item.year, 10) > 0);
              
        if (filteredApiData.length > 0) {
          // Sort by year
          filteredApiData.sort((a, b) => parseInt(a.year, 10) - parseInt(b.year, 10));
          ...
          const data = filteredApiData.map(item => {
            ...
            const powerValue = parseFloat(item.solarPower || 0);
          ...
          const legendUnit = maxTotal > 1000 ? "GWh" : "MWh";
          
          const chartFormattedData = {
            labels: labels,
            datasets: [{
              data: data,
              colors: colors, // Add colors array to dataset as functions
              color: (opacity = 1) => `rgba(0, 150, 50, ${opacity})` // Add color function to dataset
            }],
          ...
          setTotalChartData({
            ...
            datasets: [{ 
              data: [0],
              colors: [(opacity = 1) => `rgba(0, 150, 50, ${opacity})`],
              color: (opacity = 1) => `rgba(0, 150, 50, ${opacity})`
            }],
      ...
      setTotalChartError("An unexpected error occurred while fetching total data.");
      setTotalChartData({
        labels: ['Error'],
        datasets: [{ 
          data: [0],
  ...
  useEffect(() => {
    console.log(`Chart useEffect triggered. Tab: ${selectedProductionTab}, Date: ${selectedDate.toISOString()}`);
  ...
  useEffect(() => {
    if (selectedProductionTab === 'Day' && plantDayRawData && !isDayChartLoading) {
      const paramConfig = PLANT_DAY_PARAMETERS[selectedPlantDayParameterId];
      if (!paramConfig) {
        console.error('[PlantDetail Day Tab useEffect] Invalid paramConfig for id:', selectedPlantDayParameterId);
        // Use 2-hour interval labels for x-axis from 02:00 to 20:00
        const defaultChartLabels = ["02:00", "04:00", "06:00", "08:00", "10:00", "12:00", "14:00", "16:00", "18:00", "20:00"];
        setDayChartData({ labels: defaultChartLabels, datasets: [{ data: Array(37).fill(0) }] });
        setPlantDaySummaryValue('--');
      ...
      if (!Array.isArray(plantDayRawData) || plantDayRawData.length === 0) {
        console.warn('[PlantDetail Day Tab useEffect] plantDayRawData is empty or not an array.');
        setDayChartData({ labels: defaultChartLabels, datasets: [{ data: Array(48).fill(0) }] });
        setPlantDaySummaryValue('--');
      ...
      // Create half-hour buckets (30 minutes each) for full 24 hours (48 buckets total)
              // Create half-hour buckets (30 minutes each) for full 24 hours (48 buckets total)
        const halfHourBuckets = Array(48).fill(0).map(() => ({ totalValue: 0, count: 0 }));
        // Also create hour buckets for direct hour mapping
        const hourBuckets = Array(24).fill(0).map(() => ({ totalValue: 0, count: 0 }));
      ...
      const halfHourlyAverages = halfHourBuckets.map(bucket =>
        bucket.count > 0 ? parseFloat((bucket.totalValue / bucket.count).toFixed(3)) : 0
      );
  ...
  const lastNonZeroIndex = halfHourlyAverages.lastIndexOf(halfHourlyAverages.filter(v => v > 0).pop());
  
  // Create a copy of the data with nulls after the last actual data point
  ...
  const dataForDisplay = [...halfHourlyAverages];
      ...
      if (latestValidRawItemForSummary) {
          const numericSummary = paramConfig.summaryExtractor(latestValidRawItemForSummary);
          if (numericSummary !== null && !isNaN(numericSummary)) {
              setPlantDaySummaryValue(numericSummary.toFixed(paramConfig.summaryDecimalPlaces));
          } else {
              setPlantDaySummaryValue('--');
          }
      } else {
      ...
      if (selectedPlantDayParameterId === 'productionPower') {
        ...
        if (maxPower > -1 && !isNaN(peakTimeMinutes)) {
          ...
          setPeakPowerTime(`${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}`);
        } else {
          setPeakPowerWatts('--');
          setPeakPowerTime('--');
        }
        ...
        plantDayRawData.forEach(item => {
          ...
          if (!isNaN(powerW) && !isNaN(minute)) {
            powerSumForMinute[minute] = (powerSumForMinute[minute] || 0) + powerW;
            readingsCountPerMinute[minute] = (readingsCountPerMinute[minute] || 0) + 1;
          }
        });
        let totalEnergyWh = 0;
        for (const minuteKey in powerSumForMinute) {
          if (readingsCountPerMinute[minuteKey] > 0) {
            const averagePowerForThatMinuteW = powerSumForMinute[minuteKey] / readingsCountPerMinute[minuteKey];
    ...
    } else if (selectedProductionTab === 'Day' && (!plantDayRawData || plantDayRawData.length === 0) && !isDayChartLoading) {
      // Handle case where raw data is explicitly null/empty after a fetch cycle and not loading
      const paramConfig = PLANT_DAY_PARAMETERS[selectedPlantDayParameterId];
      const defaultChartLabels = ["00:00", "03:00", "06:00", "09:00", "12:00", "15:00", "18:00", "21:00"];
  ...
  }, [plantDayRawData, selectedPlantDayParameterId, selectedProductionTab, isDayChartLoading]);
    ...
    if (plant && selectedProductionTab === 'Month') {
      ...
      if (paramConfig && paramConfig.summaryExtractor) {
        ...
        if (rawValue !== undefined && rawValue !== null && !isNaN(parseFloat(rawValue))) {
          setPlantMonthSummaryValue(parseFloat(rawValue).toFixed(paramConfig.summaryDecimalPlaces));
        } else {
  ...
  useEffect(() => {
    if (plant && selectedProductionTab === 'Year') {
      ...
      if (paramConfig && paramConfig.summaryExtractor) {
        const rawValue = paramConfig.summaryExtractor(plant);
        if (rawValue !== undefined && rawValue !== null && !isNaN(parseFloat(rawValue))) {
          setPlantYearSummaryValue(parseFloat(rawValue).toFixed(paramConfig.summaryDecimalPlaces));
        } else {
  ...
  useEffect(() => {
    if (plant && selectedProductionTab === 'Total') {
      ...
      if (paramConfig && paramConfig.summaryExtractor) {
        const rawValue = paramConfig.summaryExtractor(plant);
        if (rawValue !== undefined && rawValue !== null && !isNaN(parseFloat(rawValue))) {
          setPlantTotalSummaryValue(parseFloat(rawValue).toFixed(paramConfig.summaryDecimalPlaces));
        } else {
  ...
  const chartConfigCommon = {
    ...
    // propsForDots: { // Default for LineChart if needed
    //     r: "4",
    //     strokeWidth: "1",
    //     stroke: "#005028" // Very Dark Green from Dashboard dot stroke
    // },
    propsForBackgroundLines: {
        strokeDasharray: "", // Solid lines
        stroke: "#e0e0e0", // Restored color for horizontal lines
        strokeWidth: 0.5  // Restored width
    },
    ...
    barPercentage: 0.7, // Default bar percentage
  };
  ...
  const areaChartConfig = {
    ...
    propsForBackgroundLines: {
      strokeDasharray: "", 
      stroke: "#e0e0e0", // Light grey for horizontal lines
      strokeWidth: 0.8  // Restored width
    },
    // Fix y-axis display
    formatYLabel: (value) => `${value}`, // Add W suffix directly to the labels without space
    yAxisInterval: 1,
    segments: 5, // More segments for inverter-like display
    propsForLabels: {
      fontSize: 10,
      fontWeight: '400',
      color: '#888888'
 ...
 const barChartConfig = {
    ...chartConfigCommon,
    // Customize bar chart specific props if needed
 };
 ...
 const yearlyBarChartConfig = {
    ...
    // Fix y-axis display
    formatYLabel: (value) => {
      // Apply the same logic as formatEnergyWithUnit for consistency
      const numValue = parseFloat(value);
    ...
    yAxisInterval: 1,
    segments: 4, // Add segments to ensure more y-axis labels
    propsForLabels: {
      fontSize: 10,
      fontWeight: '500'
    },
 ...
 const totalBarChartConfig = {
    ...
    propsForBackgroundLines: {
      strokeDasharray: "",
      stroke: "#e0e0e0", // Restored color for horizontal lines
      strokeWidth: 1  // Restored width
    },
    // Control the number of horizontal grid lines
    segments: 4
  };

 const monthlyBarChartConfig = {
     ...chartConfigCommon, // Inherits red bars
     decimalPlaces: 0, // Display Y-axis labels with 0 decimal places for Wh
     barPercentage: 0.25, // Thin bars for many days
     propsForLabels: {
         fontSize: 9,
         fontWeight: '500',
     },
  ...
  const handleDataPointClick = (data) => {
    ...
    try {
      ...
      const tooltipPosition = {
        x: Math.max(TOOLTIP_WIDTH/2, Math.min(safeX, safeCardWidth - TOOLTIP_WIDTH/2)), // Keep tooltip within screen width
        y: tooltipY,
        visible: true,
        value: displayValue,
        label: label,
  ...
  const getHighlightedChartData = (data, chartType) => {
    ...
    const baseColor = COLORS.primary || '#ff0000';
    const highlightColor = '#2DCE98'; // Use a visually distinct color for highlight
      ...
      datasets: [
        {
          ...
          color: (opacity = 1, index) => {
            if (selectedBarChartType === chartType && selectedBarIndex === index) {
  ...
  // Helper function to clear tooltip and selected bar
  const clearTooltipAndSelection = () => {
    setTooltipPos(prev => ({ ...prev, visible: false }));
    setSelectedBarIndex(null);
    setSelectedBarChartType(null);
  };

  // Manual click handler for bar charts (fallback) - moved to component level
  const handleBarChartClick = (event, chartType) => {
    console.log(`${chartType} chart clicked at:`, event.nativeEvent);
    
    // Get the touch coordinates
    const { pageX, pageY } = event.nativeEvent;
    
    // Calculate which bar was clicked based on position
    let chartWidth, barWidth, barIndex;
    
    if (chartType === 'Month') {
      chartWidth = Math.max(cardWidth - 32, (monthChartData?.labels?.length || 31) * 20);
      barWidth = chartWidth / (monthChartData?.labels?.length || 31);
      const relativeX = pageX - 16; // Adjust for padding
      barIndex = Math.floor(relativeX / barWidth);
    } else if (chartType === 'Year') {
      chartWidth = Math.max(cardWidth - 32, (yearChartData?.labels?.length || 12) * 40);
      barWidth = chartWidth / (yearChartData?.labels?.length || 12);
      const relativeX = pageX - 16;
      barIndex = Math.floor(relativeX / barWidth);
    } else if (chartType === 'Total') {
      // For total chart, we need to check how many bars are actually present
      const totalBars = totalChartData?.labels?.length || 1;
      chartWidth = Math.max(cardWidth - 32, totalBars * 60);
      barWidth = chartWidth / totalBars;
      const relativeX = pageX - 16;
      barIndex = Math.floor(relativeX / barWidth);
    }
    
    console.log('Calculated bar index:', barIndex);
    
    // Get the data for this bar
    let data, label, value;
    
    if (chartType === 'Month' && monthChartData && monthChartData.datasets && monthChartData.datasets[0] && monthChartData.datasets[0].data) {
      data = monthChartData.datasets[0].data[barIndex];
      label = `Day ${barIndex + 1}`;
      value = data || 0;
    } else if (chartType === 'Year' && yearChartData && yearChartData.datasets && yearChartData.datasets[0] && yearChartData.datasets[0].data) {
      data = yearChartData.datasets[0].data[barIndex];
      label = `Month ${yearChartData.labels[barIndex]}`;
      value = data || 0;
    } else if (chartType === 'Total' && totalChartData && totalChartData.datasets && totalChartData.datasets[0] && totalChartData.datasets[0].data) {
      data = totalChartData.datasets[0].data[barIndex];
      label = `Year ${totalChartData.labels[barIndex]}`;
      value = data || 0;
    }
    ...
    if (data !== undefined) {
      ...
      setSelectedBarChartType(chartType);
      ...
      // Calculate tooltip position on top of the bar
      const tooltipX = Math.max(TOOLTIP_WIDTH/2, Math.min(pageX, screenWidth - TOOLTIP_WIDTH/2));
      const tooltipY = Math.max(10, pageY - VERTICAL_OFFSET - 50); // Position above the bar
      ...
      console.log('Manual tooltip set:', { label, displayValue, x: tooltipX, y: tooltipY });
      ...
      console.log('Chart data not available for', chartType);
    }
  };
  ...
  const renderChartContent = () => {
    ...
    const renderTooltip = () => {
      ...
      return (
        <View style={[localCalendarStyles.tooltipContainer, { left: tooltipPos.x - TOOLTIP_WIDTH/2, top: tooltipPos.y }]}>
          <View style={localCalendarStyles.tooltipBubble}>
            <Text style={localCalendarStyles.tooltipText}>{tooltipPos.label}</Text>
            <Text style={localCalendarStyles.tooltipText}>{tooltipPos.value}</Text>
          </View>
             ...
             <>
              <View style={styles.miniCardsContainer}>
                <View style={styles.miniCard}>
                  <Text style={styles.miniCardLabel}>Daily Production</Text>
                  <Text style={styles.miniCardValue}>
                    {(() => {
                      if (!plantDashboardData?.daily_production) return '--';
                      const formatted = formatEnergyWithUnit(plantDashboardData.daily_production);
                ...
                <View style={styles.miniCard}>
                  ...
                  <Text style={styles.miniCardValue}>
                    {plantDashboardData?.peak_hours_today ? `${plantDashboardData.peak_hours_today} kW` : '--'}
                  </Text>
                </View>
              </View>
              
              <View style={styles.graphLegendContainer}>
                <Text style={styles.graphLegendText}>Power (kWh)</Text>
              </View>

              <View style={[styles.chartCardContainer, { alignItems: 'flex-start', paddingLeft: 0, paddingRight: 0, marginLeft: 0 }]}>
                <LineChart
                  data={{
                    ...dayChartData,
                    datasets: [{
                      ...dayChartData.datasets[0],
                      // Use visualData for line drawing only - this makes zeros not connect
                      data: dayChartData.visualData || dayChartData.datasets[0].data,
                      color: (opacity = 1) => `rgba(255, 0, 0, ${opacity})`, // Primary red
                      strokeWidth: 2
                    }]
                  }}
  ...
  chartConfig={
    ...
    backgroundGradientFrom: '#ffffff',
    backgroundGradientTo: '#ffffff',
    // Add custom strokeDashArray to hide grid lines for null values
    propsForBackgroundLines: {
      ...areaChartConfig.propsForBackgroundLines,
      strokeDasharray: "", // Solid lines
      stroke: "#e0e0e0", // Light grey for grid lines
      strokeWidth: dayChartData && dayChartData.datasets && dayChartData.datasets[0].data.some(val => val !== null) ? 0.8 : 0 // Hide grid if no data
    },
             ...
             <>
                 ...
                 <View style={styles.miniCard}>
                   ...
                   <Text style={styles.miniCardValue}>
                     {(() => {
                       ...
                       const formatted = formatEnergyWithUnit(plantDashboardData.monthlyProduction);
                       return `${formatted.value} ${formatted.unit}`;
                     })()}
                   </Text>
                 </View>
               ...
               <View style={{ position: 'relative' }}>
                 ...
                 >
                   ...
                   >
                     <BarChart
                       data={getHighlightedChartData(monthChartData, 'Month')}
                       width={Math.max(cardWidth - 32, monthChartData.labels.length * 20)}
                       height={220}
                       ...
                       getBarColor={(dataPoint, index) => {
                         ...
                         if (selectedBarChartType === 'Month' && selectedBarIndex === index) {
                           return '#2DCE98'; // Highlight color
                         }
                         return `rgba(0, 135, 90, 1)`; // Default color
                       }}
                     />
                   </TouchableOpacity>
                 </ScrollView>
              ...
              <>
                  ...
                  <View style={styles.summaryCard}>
                    ...
                    <Text style={styles.summaryValue}>
                      {(() => {
                        ...
                        const formatted = formatEnergyWithUnit(plantDashboardData.yearlyProduction);
                        return `${formatted.value} ${formatted.unit}`;
                      })()}
                    </Text>
                  </View>
                ...
                <View style={{ position: 'relative' }}>
                  ...
                  >
                    ...
                    >
                      <BarChart
                        data={getHighlightedChartData(yearChartData, 'Year')}
                        width={Math.max(cardWidth - 32, yearChartData.labels.length * 40)}
                        height={220}
                        ...
                        getBarColor={(dataPoint, index) => {
                          // Check if this bar should be highlighted
                          if (selectedBarChartType === 'Year' && selectedBarIndex === index) {
                            return '#2DCE98'; // Highlight color
                          }
                          return `rgba(0, 135, 90, 1)`; // Default color
                        }}
                  ...
                  <View style={styles.summaryCard}>
                    ...
                    <Text style={styles.summaryValue}>
                      {(() => {
                        ...
                        const formatted = formatEnergyWithUnit(plantDashboardData.totalProduction);
                        return `${formatted.value} ${formatted.unit}`;
                      })()}
                    </Text>
                  </View>
                    ...
                    >
                      <BarChart
                        data={getHighlightedChartData({
                          ...totalChartData,
                          labels: totalChartData?.labels?.map(label => String(label)) || [],
                          datasets: totalChartData?.datasets?.map(dataset => ({
                            ...dataset,
                            data: dataset.data?.map(value => Number(value)) || [],
                            color: (opacity = 1) => `rgba(0, 150, 50, ${opacity})` // Ensure color is a function
                          })) || []
                        }, 'Total')}
                        width={Math.max(cardWidth - 32, (totalChartData?.labels?.length || 1) * 60)}
                        height={220}
                        chartConfig={{
                          ...totalBarChartConfig,
                          color: (opacity = 1) => `rgba(0, 150, 50, ${opacity})`, // Ensure color is a function
                        ...
                        getBarColor={(dataPoint, index) => {
                          // Check if this bar should be highlighted
                          if (selectedBarChartType === 'Total' && selectedBarIndex === index) {
                            return '#2DCE98'; // Highlight color
                          }
                          return `rgba(0, 150, 50, 1)`; // Default color
                        }}
                      />
                    </TouchableOpacity>
                  </ScrollView>
      ...
      <View style={localCalendarStyles.dateNavigator}>
        ...
        >
          <Text style={localCalendarStyles.dateText}>
            {selectedProductionTab === 'Day' && selectedDate.toLocaleDateString()}
            {selectedProductionTab === 'Month' && selectedDate.toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}
            {selectedProductionTab === 'Year' && selectedDate.getFullYear().toString()}
            {selectedProductionTab === 'Total' && 'All Time'}
          </Text>
          <Icon name="calendar-today" size={20} color="#555" style={localCalendarStyles.calendarIcon} />
          </TouchableOpacity>
              <TouchableOpacity 
          ...
          disabled={isLoading || disableNext}
                  >
          <Icon name="chevron-right" size={28} color={disableNext ? '#ccc' : '#555'} />
              </TouchableOpacity>
        </View>
    );

    return (
      <TouchableOpacity activeOpacity={1} onPress={clearTooltipAndSelection} style={localCalendarStyles.chartAreaContainer}>
        ...
        <View style={styles.imageContainer}>
          {plant?.image ? (
            ...
            <View style={styles.placeholderImage}>
              <Icon name="eco" size={80} color="#ff0000" />
              <Text style={styles.placeholderText}>No Image</Text>
            </View>
          )}
          <View style={styles.statusBadge}>
            <Icon
              name={plantStatus.icon}
              size={20}
              color={plantStatus.color}
            />
        ...
        <InfoCard title="Plant Details" width={cardWidth}>
          <View style={styles.detailContainer}>
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{plant?.system_type || 'Not specified'}</Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Plant Type:</Text>
              <Text style={styles.detailValue}>{plant?.plant_type || 'Not specified'}</Text>
            </View>
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>
                {plant?.capacity ? (
                  <>
                    {plant.capacity}<Text style={styles.unitText}> kW</Text>
                  </>
                ) : (
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{(plant?.on_grid_date && plant.on_grid_date !== '--') ? formatDate(plant.on_grid_date) : (plant?.creation_date ? formatDate(plant.creation_date) : 'Not specified')}</Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Last Updated:</Text>
              <Text style={styles.detailValue}>{(plant?.last_update && plant?.last_update !== '--') ? plant.last_update : 'Not specified'}</Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>Creation Date:</Text>
              <Text style={styles.detailValue}>{plant?.creation_date ? formatDate(plant.creation_date) : 'Not specified'}</Text>
            </View>
            <View style={styles.detailRow}>
              <Text style={styles.detailLabel}>On-Grid Date:</Text>
              <Text style={styles.detailValue}>{(plant?.on_grid_date && plant.on_grid_date !== '--') ? formatDate(plant.on_grid_date) : 'Not specified'}</Text>
            </View>
            {(plant?.azimuth !== undefined && plant?.azimuth !== null) && (
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Azimuth:</Text>
                <Text style={styles.detailValue}>
                  <>
                    {plant.azimuth}<Text style={styles.unitText}>°</Text>
                  </>
                </Text>
              </View>
            )}
            {(plant?.tilt_angle !== undefined && plant?.tilt_angle !== null) && (
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>Tilt Angle:</Text>
                <Text style={styles.detailValue}>
                  <>
                    {plant.tilt_angle}<Text style={styles.unitText}>°</Text>
                  </>
                </Text>
              </View>
            )}
        ...
        <InfoCard title="Location" width={cardWidth}>
          <View style={styles.detailContainer}>
            ...
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailValue}>{plant?.address || 'Not specified'}</Text>
            </View>
            {plant?.city && (
              <View style={styles.detailRow}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => {
            if (plant && (plant._id || plant.id)) {
              ...
              Alert.alert("Error", "Plant data is not available to show details.");
            }
          }}
        >
              ...
              {['Day', 'Month', 'Year', 'Total'].map((tabName) => (
                <TouchableOpacity 
                  ...
                  style={[
                    ...
                    selectedProductionTab === tabName && localCalendarStyles.calendarTabActive
                  ]} 
                ...
                >
                  <Text 
                    style={[
                      ...
                      selectedProductionTab === tabName && localCalendarStyles.calendarTabTextActive
                    ]}>
                    {tabName}
                  </Text>
                </TouchableOpacity>
...
const localCalendarStyles = StyleSheet.create({
  ...
  navButton: { // Style for arrow buttons
     padding: 5,
  },
  dateDisplayButton: { // Style for the touchable date and calendar icon
      flexDirection: 'row',
      alignItems: 'center',
      paddingHorizontal: 10, 
      paddingVertical: 5,
  ...
  dayParameterContainer: { // Renamed from dayParameterContainer to avoid conflict if it existed elsewhere
    flexDirection: 'row',
  ...
  parameterButton: { // Renamed from parameterButton to avoid conflict
    flexDirection: 'row',
    alignItems: 'center',
  ...
  parameterButtonText: { // Renamed from parameterButtonText
    fontSize: 14,
