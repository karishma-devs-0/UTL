...
const PlantAboutScreen = ({ route }) => {
  ...
  const renderTooltip = () => {
    ...
    return (
      <View style={[styles.tooltipContainer, { left: tooltipPos.x - 40, top: tooltipPos.y - 60 }]}>
        ...
        <View style={{ flex: 1, position: 'relative' }}>
          ...
          >
            <ScrollView contentContainerStyle={styles.scrollContentContainer}> 
                {plantInfo.imageUrl ? (
                    <Image source={{ uri: plantInfo.imageUrl }} style={styles.image} resizeMode="cover" />
                ) : (
                    <View style={[styles.image, styles.placeholderImage]}>
                        <Icon name="eco" size={50} color="#00875A" /> 
                    </View>
                )}
                <Text style={styles.name}>{plantInfo.name || 'Plant Name Unavailable'}</Text>
              <View style={styles.summaryCard}>
                <InfoCard title="Basic Information" widthStyle={styles.cardWidth}>
                    ...
                    {renderDetailRow("Created On", plantInfo.creation_date ? formatDate(plantInfo.creation_date) : null)}
                    {renderDetailRow("On-Grid Date", plantInfo.on_grid_date ? formatDate(plantInfo.on_grid_date) : null)}
                ...
                {/* Interactive Summary Card with Tooltips */}
                  ...
                  <View style={styles.summaryGrid}>
                    <TouchableOpacity 
                      ...
                      onPress={(event) => handleTooltipClick("Capacity", `${plantInfo.capacity || 'N/A'} kWp`, event)}
                    >
                      <Icon name="flash-on" size={24} color="#00875A" />
                      <Text style={styles.summaryLabel}>Capacity</Text>
                      <Text style={styles.summaryValue}>{plantInfo.capacity || 'N/A'} kWp</Text>
                    </TouchableOpacity>
                    
                    <TouchableOpacity 
                      style={styles.summaryItem}
                      onPress={(event) => handleTooltipClick("Plant Type", plantInfo.plant_type || 'N/A', event)}
                    >
                      <Icon name="eco" size={24} color="#00875A" />
                      <Text style={styles.summaryLabel}>Type</Text>
                      <Text style={styles.summaryValue}>{plantInfo.plant_type || 'N/A'}</Text>
                    </TouchableOpacity>
                    
                    <TouchableOpacity 
                      style={styles.summaryItem}
                      onPress={(event) => handleTooltipClick("Location", plantInfo.location || 'N/A', event)}
                    >
                      <Icon name="location-on" size={24} color="#00875A" />
                      <Text style={styles.summaryLabel}>Location</Text>
                      <Text style={styles.summaryValue}>{plantInfo.location || 'N/A'}</Text>
                    </TouchableOpacity>
                    
                    <TouchableOpacity 
                      style={styles.summaryItem}
                      onPress={(event) => handleTooltipClick("Status", "Active", event)}
                    >
