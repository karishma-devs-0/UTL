...
const formatTimestamp = (isoString) => {
  ...
  try {
    ...
    if (isNaN(date.getTime())) {
      const modifiedIsoString = isoString.replace(' ', 'T') + (isoString.endsWith('Z') ? '' : 'Z');
      const modifiedDate = new Date(modifiedIsoString);
      if (isNaN(modifiedDate.getTime())) return isoString;
      return modifiedDate.toLocaleString([], { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    }
...
const DevicesPage = ({ navigation, route }) => {
  ...
  const fetchDevices = useCallback(async () => {
    ...
    console.log(`DevicesPage: Fetching devices for plantId: ${plantId} and tab: ${activeTab}`);
    setLoading(true);
    setError(null);
    ...
    try {
        ...
        if (activeTab === 'Logger') {
            ...
            result = { success: false, error: `Unsupported device type: ${activeTab}` };
        }
  ...
  const fetchAllDeviceSummary = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        console.log(`[Devices] Successfully fetched ${inverter?.length || 0} inverters and ${logger?.length || 0} loggers`);
        ...
        console.error("[Devices] Failed to fetch device summary:", result.error);
        setAllInvertersError(result.error || 'Failed to load device summary.');
        setAllInverters([]);
        setAllLoggers([]);
      }
    } catch (error) {
      console.error("[Devices] Unexpected error in fetchAllDeviceSummary:", error);
      setAllInvertersError('An unexpected error occurred while fetching device summary.');
      setAllInverters([]);
      setAllLoggers([]);
    } finally {
  ...
  useEffect(() => {
    ...
    if (isFocused) { // Re-fetch when screen comes into focus
  ...
  // --- Filtering Logic ---
  const filteredInverters = allInverters.filter(inverter => 
  ...
  const filteredLoggers = allLoggers.filter(logger => 
  ...
  const handleViewDevice = (device, type) => {
    ...
    if (type === 'inverter') {
      ...
      if (loggerIdToPass && typeof associatedPlantId !== 'undefined') {
        console.log(`[Devices.js/handleViewDevice] Navigating to LoggerTabs with loggerId: ${loggerIdToPass}, plantId: ${associatedPlantId}`);
        navigation.navigate('LoggerTabs', { loggerId: loggerIdToPass, plantId: associatedPlantId });
      } else {
        console.warn('[Devices.js/handleViewDevice] Cannot navigate to logger: No valid S/N or plantId found. Device:', device);
        Alert.alert(
          "Navigation Error",
  ...
  const renderDeviceItem = ({ item }) => {
    const deviceName = item.name || item.deviceName || `Device S/N: ${item.sno || 'Unknown'}`;
      ...
      >
        <View style={localStyles.deviceHeader}>
          ...
          <View style={localStyles.deviceStatusContainer}>
            <Text style={[localStyles.deviceStatus, getStatusStyle(deviceStatus)]}>
              {deviceStatus}
            </Text>
        ...
        <View style={localStyles.deviceDetails}>
          <View style={localStyles.deviceDetailItem}>
            ...
            <Text style={localStyles.deviceDetailValue}>{deviceTypeDisplay}</Text>
          </View>
          <View style={localStyles.deviceDetailItem}>
  ...
  const renderEmptyState = () => {
    ...
    return (
      <View style={localStyles.emptyStateContainer}>
        ...
        {plantId ? (
            <Text style={localStyles.emptyStateSubText}>Could not fetch {activeTab.toLowerCase()} list for this plant.</Text>
        ) : (
             <Text style={localStyles.emptyStateSubText}>No Plant ID selected.</Text>
        )}
  ...
  const renderInverterItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity 
        style={localStyles.deviceCard} 
        onPress={() => navigation.navigate('InverterTabs', { 
            deviceData: item 
        })}
      >
        <View style={localStyles.deviceHeader}>
          <Icon name="settings-input-hdmi" size={24} color="#00875A" style={localStyles.deviceIcon} />
          <View style={localStyles.deviceHeaderText}>
            <Text style={localStyles.deviceName}>{getSafeValue(item.name, 'Unnamed Inverter')}</Text>
            <Text style={localStyles.deviceSerial}>S/N: {getSafeValue(item.inverter_sno)}</Text>
          </View>
        </View>
        <View style={localStyles.deviceBody}>
           <View style={localStyles.dataItem}>
               <Text style={localStyles.dataLabel}>Power</Text>
               <Text style={localStyles.dataValue}>{getSafeValue(item.solar_power)} kW</Text>
           </View>
           <View style={localStyles.dataItem}>
               <Text style={localStyles.dataLabel}>Daily Prod.</Text>
               <Text style={localStyles.dataValue}>{getSafeValue(item.daily_production)} kWh</Text>
           </View>
           <View style={localStyles.dataItem}>
               <Text style={localStyles.dataLabel}>Capacity</Text>
               <Text style={localStyles.dataValue}>{getSafeValue(item.module_capacity)} kWp</Text>
           </View>
        </View>
         <View style={localStyles.deviceFooter}>
            <Text style={localStyles.footerText}>Last Updated: {lastUpdated}</Text>
         </View>
      </TouchableOpacity>
    );
  };
  ...
  const renderLoggerItem = ({ item }) => {
    ...
    const deviceName = item.name || item.sno || 'Unnamed Logger';
      ...
      <TouchableOpacity 
        ...
        onPress={() => {
          if (loggerIdToPass && associatedPlantId != null) { 
            ...
            Alert.alert("Navigation Error", `Could not open logger details. Missing S/N or Plant ID.`);
          }
        }}
      >
        {/* Main content of the card */}
        <View style={localStyles.deviceHeader}> 
          <Icon name="router" size={24} color="#4CAF50" style={localStyles.deviceIcon} /> 
          <View style={localStyles.deviceHeaderText}>
            <Text style={localStyles.deviceName} numberOfLines={1}>{deviceName}</Text>
            <Text style={localStyles.deviceSerial}>S/N: {item.sno || 'N/A'}</Text>
          </View>
  ...
  const renderListEmptyComponent = () => {
    ...
    const dataLength = activeTab === 'Inverter' ? filteredInverters.length : filteredLoggers.length;
    ...
    if (isLoading && !isRefreshing) { 
      return (
        <View style={localStyles.emptyStateContainer}>
          <ActivityIndicator size="large" color="#00875A" />
          <Text style={localStyles.emptyStateText}>Loading {deviceType.toLowerCase()}s...</Text>
        </View>
      );
    }
    if (error) {
       return (
         <View style={localStyles.emptyStateContainer}>
           ...
           <TouchableOpacity style={localStyles.retryButton} onPress={fetchAllDeviceSummary}>
     ...
     if (!isLoading && dataLength === 0) {
       const iconName = deviceType === 'Inverter' ? "settings-input-hdmi" : "router";
       return (
          <View style={localStyles.emptyStateContainer}>
             <Icon name={iconName} size={40} color="#ccc" />
             <Text style={localStyles.emptyStateText}>No {deviceType.toLowerCase()}s found.</Text>
             {searchQuery ? <Text style={localStyles.emptyStateSubText}>Try adjusting your search.</Text> : null}
          </View>
       );
     }
    ...
    <SafeAreaView style={localStyles.container}>
      ...
      {activeTab === 'Inverter' && (
        <FlatList
          ...
          contentContainerStyle={
            filteredInverters.length === 0 ? { flexGrow: 1 } : { paddingBottom: 20, paddingTop: 10 }
          }
           refreshControl={
             <RefreshControl
      ...
      {activeTab === 'Logger' && (
        <FlatList
          ...
          ListEmptyComponent={renderListEmptyComponent}
          contentContainerStyle={
             filteredLoggers.length === 0 ? { flexGrow: 1 } : { paddingBottom: 20, paddingTop: 10 }
          }
           refreshControl={
             <RefreshControl
               refreshing={isRefreshing}
