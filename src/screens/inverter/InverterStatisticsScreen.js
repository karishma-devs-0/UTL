...
const PARAMETER_GROUPS = [
  ...
  { id: 'DC_CURRENT_VOLTAGE', label: 'DC Current & Voltage', parameters: DC_CURRENT_VOLTAGE_PARAMETERS }
];
...
const InverterStatisticsScreen = ({ route }) => {
  ...
  console.log('[InverterStatisticsScreen] Received route.params:', JSON.stringify(route.params));
  console.log('[InverterStatisticsScreen] Extracted deviceData:', JSON.stringify(deviceData));

  // Get device ID, handling all possible property names and formats
  const deviceId = deviceData?.loggerSno || deviceData?.logger_sno || deviceData?.sno || 
                  deviceData?.sn || deviceData?.device_sn;
  
  // Get inverter ID for the AppBar title
  const inverterId = deviceData?.inverterSno || deviceData?.inverter_sno || deviceData?.sno || deviceData?.id || '';
  
  // Ensure deviceId is properly formatted (should start with '&')
  const formattedDeviceId = deviceId ? (deviceId.startsWith('&') ? deviceId : `${deviceId}`) : null;
  ...
  const [selectedParameterGroup, setSelectedParameterGroup] = useState('AC_DATA');
  const [selectedParameters, setSelectedParameters] = useState(Object.keys(AC_DATA_PARAMETERS));
  ...
  const aggregateDataByInterval = useCallback((data) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      console.log('[InverterStatisticsScreen] Fetching data for device:', formattedDeviceId);
      console.log('[InverterStatisticsScreen] Selected date:', formatDateForAPI(selectedDate));
      
      const response = await apiClient.post('charts/devices/daily/device_daily_chart', {
        device_sn: formattedDeviceId,
        date_parameter: formatDateForAPI(selectedDate)
      });

      console.log('[InverterStatisticsScreen] Raw API Response:', JSON.stringify(response.data, null, 2));
      ...
      console.log('[InverterStatisticsScreen] Aggregated Data:', JSON.stringify(aggregatedData, null, 2));
      
      if (!aggregatedData || aggregatedData.length === 0) {
        console.log('[InverterStatisticsScreen] No valid data after aggregation');
        setChartData(null);
      ...
      if (!hasValidData) {
        console.log('[InverterStatisticsScreen] No valid data points for selected parameters');
        setChartData(null);
      ...
      const datasets = selectedParameters.map(paramKey => {
        try {
          // Create a map of hour to value
          const hourMap = {};
          ...
          return {
            data,
            color: (opacity = 1) => PARAMETER_GROUPS
              .find(group => group.id === selectedParameterGroup)
      ...
      if (datasets.length === 0) {
        console.log('[InverterStatisticsScreen] No valid datasets after processing');
        setChartData(null);
      ...
      if (!hasAnyValidData) {
        console.log('[InverterStatisticsScreen] No valid data in final formatted data');
        setChartData(null);
        setError('No valid data available for plotting');
        return;
      }

      console.log('[InverterStatisticsScreen] Final Chart Data:', JSON.stringify(formattedData, null, 2));
  ...
  }, [formattedDeviceId, selectedDate, selectedParameters, selectedParameterGroup, aggregateDataByInterval]);
  ...
  const fetchMonthlyData = useCallback(async () => {
    ...
    try {
      const response = await apiClient.post('charts/devices/monthly/device_month_chart', {
        device_sn: formattedDeviceId,
  ...
  const fetchTotalData = useCallback(async () => {
    ...
    try {
      const response = await apiClient.post('charts/devices/total/device_total_chart', {
        device_sn: formattedDeviceId,
        date_parameter: formatYearParameter(selectedDate)
      });

      console.log('Total API Response:', JSON.stringify(response.data, null, 2));

      if (response.data?.results?.length > 0) {
        // Get the production value directly from the API response
        const productionValue = parseFloat(response.data.results[0].production) || 0;
  ...
  const fetchYearlyData = useCallback(async () => {
    ...
    try {
      const response = await apiClient.post('charts/devices/yearly/device_year_chart', {
        device_sn: formattedDeviceId,
        date_parameter: formatYearParameter(selectedDate)
      });

      console.log('Yearly API Response:', JSON.stringify(response.data, null, 2));
  ...
  }, [selectedTab, fetchData, fetchMonthlyData, fetchYearlyData, fetchTotalData]);
  ...
  const handleParameterGroupSelect = (groupId) => {
    ...
    const groupParameters = Object.keys(PARAMETER_GROUPS.find(g => g.id === groupId).parameters);
  ...
  const formatChartData = (data, parameters) => {
    ...
    const datasets = parameters.map((paramKey, index) => {
      const paramInfo = PARAMETER_GROUPS
        .find(group => group.id === selectedParameterGroup)
        .parameters[paramKey];
    ...
    <SafeAreaView style={styles.mainContainer} edges={['top', 'bottom']}>
        ...
        {!formattedDeviceId && (
          <View style={styles.errorBanner}>
            ...
            <Text style={styles.errorBannerText}>
              Device ID is missing. Please ensure the device is properly selected.
            </Text>
          </View>
        )}
              ...
              <View style={styles.parameterGroupBar}>
                {PARAMETER_GROUPS.map((group) => (
                    ...
                    style={[
                      ...
                      selectedParameterGroup === group.id && styles.activeParameterGroup
                    ]}
                        onPress={() => handleParameterGroupSelect(group.id)}
                  >
                    <Text style={[
                      styles.parameterGroupText,
                      selectedParameterGroup === group.id && styles.activeParameterGroupText
                    ]}>
                      {group.label}
                        </Text>
                    </TouchableOpacity>
        ...
        <View style={styles.dateBar}>
          ...
          <TouchableOpacity onPress={() => setDatePickerVisibility(true)}>
            <Text style={styles.dateText}>
              ...
              {selectedTab === 'Month' && selectedDate.toLocaleDateString(undefined, { year: 'numeric', month: 'long' })}
          ...
          {isLoading ? (
            ...
            <>
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                {selectedTab === 'Day' ? (
                  <View style={{ width: Math.max(screenWidth * 1.1, 25 * 28), height: 220 }}>
                    <LineChart
                      data={chartData}
                  ...
                  <BarChart
                    data={chartData}
                    width={Math.max(screenWidth, chartData.labels.length * (selectedTab === 'Month' ? 30 : 100))}
                    height={220}
                    chartConfig={{
                      ...(selectedTab === 'Month' ? monthChartConfig : yearChartConfig),
                      color: (opacity = 1) => `rgba(220, 53, 69, ${opacity})`, // Changed to red
                      propsForBackgroundLines: {
                        strokeDasharray: "",
                        stroke: "#e0e0e0",
              ...
              {selectedTab !== 'Day' && (
                <View style={styles.unitLegendContainer}>
                  <View style={styles.unitLegendItem}>
                    <View style={[styles.legendColor, { backgroundColor: '#dc3545' }]} /> {/* Changed to red */}
                    <Text style={styles.legendText}>Production ({TAB_UNITS[selectedTab]})</Text>
                  </View>
                </View>
              )}
            </>
      ...
      return (
                <View key={param} style={styles.legendItem}>
                  <View style={[styles.legendColor, { backgroundColor: paramInfo.color }]} />
        ...
        <DateTimePickerModal
          ...
          mode={selectedTab === 'Year' ? 'date' : selectedTab === 'Month' ? 'date' : 'date'}
          onConfirm={(date) => {
          ...
          >
            <View style={styles.modalOverlay}>
              <View style={styles.modalContent}>
                ...
                <FlatList
                  data={Object.entries(PARAMETER_GROUPS.find(g => g.id === selectedParameterGroup).parameters)}
